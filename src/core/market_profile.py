...
class MarketProfile:
    ...
    region: str  # "cn" | "us" | "hk"
    # 用于判断整体走势的指数代码，cn 用上证 000001，us 用标普 SPX，hk 用恒生科技 HSTECH
    mood_index_code: str
    # 新闻搜索关键词
    news_queries: List[str]
    # 指数点评 Prompt 提示语
    prompt_index_hint: str
    # 市场概况是否包含涨跌家数、涨停跌停（A 股/港股有，美股无）
    has_market_stats: bool
    # 市场概况是否包含板块涨跌（A 股/港股有，美股暂无）
...
HK_PROFILE = MarketProfile(
    region="hk",
    mood_index_code="HSTECH",
    news_queries=[
        "港股 大盘",
        "恒生指数",
        "香港股市",
    ],
    prompt_index_hint="分析恒生指数、恒生科技、国企指数等港股主要指数走势特点",
    has_market_stats=True,
    has_sector_rankings=True,
)
